...
ruby ${enginepath}/elastic-mapreduce --create --stream --args -input ${inputdirb} --alive --num-instances=4 --enable-debugging --log-uri s3://cciemorylog/ --master-instance-type=m1.medium  --name "Joiner MR"  --mapper s3://cciemory/program/tagmapper.py --reducer "s3://cciemory/program/${reducecmd}" --input ${inputdira} --output s3n://cciemory/hadoopgis/outputjoin/  --jobconf mapred.reduce.tasks=${reducecount} --bootstrap-action "s3://cciemory/bootstrap/bootcopygeosspatial.sh"
